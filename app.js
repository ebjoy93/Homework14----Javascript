...
function filterClick()
{
    ...
    data.forEach(function(data_i) 
    {
        ss=Object.values(data_i);if(ss[0]==filter_date)
        {
            var row=tbody.append('tr');
...
for (var i=0;i<5;i++)
{
    MoreFilter.append("button").attr("type","button").attr("class","btn btn-primary").attr("id",moreLabels_arr[i]+"filter").text(moreLabels_arr[i])
}
...
function create_filter_table(filter_kind,filter_value)
{
    ...
    filterCondition.append('br')
    var table1 = select0.append('div').attr("class","col-md-12").append('div').attr("id","table-area").style("text-align", "center").append('table').attr('class','table-striped').style("width","100%")
    ...
    thead.append('tr').selectAll('th').style("min-width","90px").data(columns).enter().append('th').text(function (column)
        { 
            return column;
        });

    tableData.forEach(function(data_i)
    {
        ss=Object.values(data_i);if(ss[filter_kind]==filter_value)
        {
            var row=tbody.append('tr');
            ss.forEach((info)=>row.append('td').text(info));
        };
...
function moreFilterClick()
{
    ...
    if (!str.length<1)
    {
        filter_key="#"+str; filter_kind= moreFilter_arr.indexOf(str)
    }
...
// throw new Error("somthing went wrong!")
    ...
    data_3.unshift(" ")
    var select = d3.select(filter_key).append('select').attr('class','select').on('change',onchange)
